...
Unpacked unpack(in ubyte[] buffer)
{
    ...
    try
    {
	unpacker = StreamingUnpacker(buffer);
    }
    catch (Exception e)
    {
	throw new UnpackException("Deserialization failure" ~ e.msg);
    }
...
Type unpack(Type, bool withFieldName = false)(in ubyte[] buffer)
{
    ...
    try
    {
	unpacker = Unpacker(buffer, buffer.length, withFieldName);
    }
    catch (Exception e)
    {
	throw new UnpackException("unpack in msgpack error:: " ~ e.msg);
    }
...
else
{
	...
	private:
		...
		{
		    throw new UnpackException("initializeBuffer error:: " ~ e.msg);
		}
		used_   = size;
...
struct StreamingUnpacker
{
	...
	this(in ubyte[] target, in size_t bufferSize = 8192)
	{
	    ...
	    {
		throw new UnpackException("StreamingUnpacker this error:: " ~ e.msg);
	    }
	    initializeContext();
	}
